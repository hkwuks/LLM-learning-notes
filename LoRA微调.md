...
$$
- 用**随机高斯分布初始化A**，用**0矩阵初始化B**，保证训练的开始此旁路矩阵仍然是0矩阵。
...
- 动态监控
  - 梯度范数：过高的梯度需要启用梯度裁剪，理想值(0.1,1.0)
  - 损失曲线：若震荡明显，降低学习率或者增加Batch Size

- 两阶段训练：

    | 阶段     | 参数策略                 | 目标             |
    | -------- | ------------------------ | ---------------- |
    | 探索阶段 | batch=32, LR=3e-4, r=8   | 快速搜索参数空间 |
    | 收敛阶段 | batch=256, LR=1e-5, r=16 | 精细调优         |
