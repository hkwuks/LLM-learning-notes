...
def GroupQueryAttention(X, mask=None, n_heads=8, n_groups=2):
    '''
        实现多查询组共享注意力机制
        Args:
    ...
    w_k = np.random.randn(d_model, d_k * n_groups)
    w_v = np.random.randn(d_model, d_k * n_groups)
    ...
    K = K.reshape(batch_size, seq_len, n_groups, d_k).repeat(n_heads // n_groups, axis=2).transpose(0, 2, 1, 3)
    V = V.reshape(batch_size, seq_len, n_groups, d_k).repeat(n_heads // n_groups, axis=2).transpose(0, 2, 1, 3)
...
output = GroupQueryAttention(matrix)
print(output)
print(output.shape)
