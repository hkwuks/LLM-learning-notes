...
def SparesAttention(X, mask=None, n_heads=8, window_size=3):
    '''
    实现稀疏注意力机制
    Args:
    ...
    # 带状注意力机制
    sparse_mask = np.zeros([batch_size, n_heads, seq_len, seq_len])
    for i in range(seq_len):
        start = max(0, i - window_size)
        end = min(i + window_size + 1, seq_len)
        sparse_mask[:, :, i, start:end] = 1
    scores = np.where(sparse_mask == 0, scores, -np.inf)
